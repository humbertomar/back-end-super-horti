...
class Model_pedidos extends CI_Model
{
	...
	public function getPedidosData($id = null)
	{
		...
		if($user_id == 1) {
			...
			$sql = "SELECT * FROM pedidos WHERE store_id = ? ORDER BY id DESC";
	...
	public function getPedidosItemData($pedido_id = null)
	{
		if(!$pedido_id) {
			return false;
		}
		...
		$query = $this->db->query($sql, array($pedido_id));
		return $query->result_array();
	}
	...
	public function create()
	{
    	...
    	for($x = 0; $x < $count_product; $x++) {
    		...
    		$this->db->insert('pedido_items', $items);
    	}
	...
	public function countPedidoItem($pedido_id)
	{
		if($pedido_id) {
			$sql = "SELECT * FROM pedido_items WHERE pedido_id = ?";
			$query = $this->db->query($sql, array($pedido_id));
	...
	public function update($id)
	{
		if($id) {
			...
			$pedido_data = $this->getPedidosData($id);
			$data = $this->model_tables->update($pedido_data['table_id'], array('available' => 1));
			...
			// now remove the pedido item data 
	...
	public function remove($id)
	{
		if($id) {
			...
			$this->db->where('pedido_id', $id);
			$delete_item = $this->db->delete('pedido_items');
			return ($delete == true && $delete_item) ? true : false;
		}
	}

	public function countTotalPaidPedidos()
	{
